...
// used by CreateOrBindGPUTexture for depth map
...
float DepthImage::RGBIntToFloat(uint8_t val) {
  double castToDouble = val/255.0f;

  /*if (castToDouble > FLT_MAX) {
    return FLT_MAX;
  }
  if (castToDouble < FLT_MIN) {
    return  FLT_MIN;
  }*/
  return val/255.0f;
}
...
bool DepthImage::CreateOrBindCPUTexture() {
  if (cpu_texture_id_) {
    ...
    glTexImage2D(GL_TEXTURE_2D, 0,  // mip-map level
                 GL_RGB, rgb_camera_intrinsics_.width,
                 rgb_camera_intrinsics_.height, 0,  // border
...
void DepthImage::RenderDepthMap(glm::mat4& color_t1_T_depth_t0,
...
void DepthImage::RenderRGBPointCloud(
  ...
  PointCloudData* pcd = new rgb_depth_sync::PointCloudData();
  ...
  rgb_pcd_buffer_.resize(depth_image_size);
  std::fill(rgb_pcd_buffer_.begin(), rgb_pcd_buffer_.end(), 0.0f);
  std::vector<float> transformed_unordered_point_cloud_to_image_frame;
  ...
  for (size_t i = 0; i <= point_cloud_size-3; i = i+3) {
    ...
    if (pixel_x > depth_image_width || pixel_y > depth_image_height || pixel_x < 0 || pixel_y < 0) {
      continue;
    }
    ...
    if (index >= render_rgb_image_buffer.size()) {
      ...
      transformed_unordered_point_cloud_to_image_frame.push_back(color_t1_point.x);
      transformed_unordered_point_cloud_to_image_frame.push_back(color_t1_point.y);
      transformed_unordered_point_cloud_to_image_frame.push_back(color_t1_point.z);
      ...
      transformed_unordered_point_cloud_to_image_frame.push_back(*reinterpret_cast<float *>(&tmp));
    }
  }

  if(savePCDtoFile || sendPCDviaSocket) {
    pcd->setPCDData(transformed_unordered_point_cloud_to_image_frame, translation, rotation, timestamp);
    pcd->setUnordered();
  }
  if(savePCDtoFile) {
    pcd->saveToFile();
  ...
  if(sendPCDviaSocket) {
    pcd->saveToSocket(socket_addr_, socket_port_);
  }
}
...
void DepthImage::RenderRGBMap(
  ...
  rgb_display_buffer_.resize(depth_image_size*3);
  ...
  for (size_t i = 0; i <= point_cloud_size-3; i = i+3) {
    ...
    r = rgb_buffer[index*3];
    g = rgb_buffer[index*3 + 1];
    b = rgb_buffer[index*3 + 2];

    float depth_value = color_t1_point.z;

    UpSampleRGBAroundPoint(r, g, b, depth_value, pixel_x, pixel_y,
                           &rgb_display_buffer_, &depth_map_buffer_);
  }

  /*if(store_ordered_point_cloud_) {
    ...
    for (size_t v = 0; v < depth_image_height; v++) {
      for (size_t u = 0; u < depth_image_width; u++) {
        ...
        float rgb = rgb_pcd_buffer_[index];
  ...
  glTexImage2D(GL_TEXTURE_2D, 0,  // mip-map level
               GL_RGB, depth_image_width, depth_image_height, 0,  // border
               GL_RGB, GL_FLOAT, rgb_display_buffer_.data());
  glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
  glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
...
void DepthImage::UpSampleRGBAroundPoint(uint8_t r, uint8_t g, uint8_t b, float depth_value,
  ...
  for (int a = -kWindowSize; a <= kWindowSize; ++a) {
    for (int b = -kWindowSize; b <= kWindowSize; ++b) {
      ...
      if (pixel_num > 0 && pixel_num < image_size) {
        ...
        // ordered point cloud
        //(*rgb_pcd_buffer)[pixel_num] = rgb_value;
        // rgb map
        /*(*rgb_display_buffer)[pixel_num*3] = RGBIntToFloat(r);
        (*rgb_display_buffer)[pixel_num*3+1] = RGBIntToFloat(g);
        (*rgb_display_buffer)[pixel_num*3+2] = RGBIntToFloat(b);*/
        (*rgb_display_buffer)[pixel_num*3] = r/255.0f;
        (*rgb_display_buffer)[pixel_num*3+1] = g/255.0f;
        (*rgb_display_buffer)[pixel_num*3+2] = b/255.0f;
      }
    }
  }
}

void DepthImage::UpSampleDepthAroundPoint(uint8_t grayscale_value, float depth_value, int pixel_x, int pixel_y,
