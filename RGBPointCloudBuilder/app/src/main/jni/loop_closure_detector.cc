...
namespace rgb_depth_sync {
  ...
  LoopClosureDetector::LoopClosureDetector(PCDContainer* pcd_container) {
    ...
    orb_dist_th_ = 64;
    //poses_ = pcl::PointCloud<pcl::PointXYZ>::Ptr(new pcl::PointCloud<pcl::PointXYZ>);
  }

  LoopClosureDetector::~LoopClosureDetector() {}

  /*void LoopClosureDetector::AddTranslation(glm::vec3 translation, int id) {
    pcl::PointXYZ p;
    p.x = translation[0];
    p.y = translation[1];
    p.z = translation[2];
    poses_->points.push_back(p);
  }

  vector<int> LoopClosureDetector::ClosestPoses(int lastIndex) {
    pcl::search::KdTree< pcl::PointXYZ> kdTree;
    std::vector< int > k_indices;
    std::vector< float > k_sqr_distances;
    kdTree.setInputCloud(poses_);
    pcl::PointXYZ p;
    p.x = pcd_container_->pcd_container_[lastIndex]->GetTranslation()[0];
    p.y = pcd_container_->pcd_container_[lastIndex]->GetTranslation()[0];
    p.z = pcd_container_->pcd_container_[lastIndex]->GetTranslation()[0];
    kdTree.nearestKSearch(p, 15, k_indices, k_sqr_distances);
  ...
  void LoopClosureDetector::ComputeAllSM(int lastIndex) {
    ...
    for (int current = 0; current < lastIndex; current++) {
      ...
      // add last previous to neighbors list
